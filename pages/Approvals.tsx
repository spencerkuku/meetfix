...
export const Approvals: React.FC = () => {
  const { bookings, rooms, updateBooking, removeBooking } = useData();
            ...
            return (
              <div key={booking.id} className="bg-white rounded-xl border border-l-4 border-l-yellow-400 shadow-sm p-6 flex flex-col gap-4 animate-slide-up hover:shadow-md transition-shadow">
                ...
                <div className="flex justify-end gap-3 mt-auto pt-2">
                    <Button variant="secondary" className="text-red-600 hover:bg-red-50 hover:text-red-700 border-transparent" onClick={() => handleReject(booking.id)}>
                        <XCircle size={16} className="mr-1"/> 拒絕
                    </Button>
                    <Button className="bg-green-600 hover:bg-green-700 text-white shadow-sm hover:shadow" onClick={() => handleApprove(booking.id)}>
